...
class LocA:
    def __init__(self, INPUT_FILE, KAKAO_API_KEY):
        ...
        file_path = "nearest_cache.json"
        if os.path.exists(file_path):
            try:
                os.remove(file_path)
                print(f"'{file_path}' Successfully Remove cahce file..!")
            except OSError as e:
                print(f"Error: '{file_path}' Failed Remove cache file. {e}")
        else:
            print(f"'{file_path}' Cache file does not exist.")

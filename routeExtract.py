...
class Extractor:
    ...
    def get_route(self, start_lat, start_lon, end_lat, end_lon):
        ...
        try:
            ...
            if "routes" in result and result["routes"] and "sections" in result["routes"][0] and result["routes"][0]["sections"]:
                ...
                for section in route["sections"]:
                    for road in section["roads"]:
                        ...
                        # 카카오 API는 (경도, 위도) 순서로 반환 (vertexes[i], vertexes[i+1] = lon, lat)
            ...
            pass # 카카오 API 실패 시 오류 출력 없이 OSMnx로 넘어감

        # 카카오 API가 실패한 경우 none return
        return None
...
if __name__ == '__main__':
    ...
    dep_acc_data = {
        'lat': snap_dep_lat,    # 시작 위도 (출발지)
        'lon': snap_dep_lon, # 시작 경도 (출발지)
        'dest_lat': snap_acc_lat,  # 목적지 위도 (사고지점)
        'dest_lon': snap_acc_lon   # 목적지 경도 (사고지점)
    }
    acc_dst_data = {
        'lat': snap_acc_lat,    # 시작 위도 (사고지점)
        'lon': snap_acc_lon, # 시작 경도 (사고지점)
        'dest_lat': snap_dst_lat,  # 목적지 위도 (목적지)
        'dest_lon': snap_dst_lon# 목적지 경도 (출발지)
    }
